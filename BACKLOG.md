...
## Auth — Session/refresh-token store ✅ Hoàn tất (2026-07-22)

Đóng lại — không còn hoãn. Xem chi tiết trong `PLAN.md` mục "Auth-hardening (2026-07-22)".
- Bảng `RefreshToken` (migration `016_refresh_token`) lưu SHA-256 hash refresh token theo user.
- `/auth/refresh` verify JWT + tra DB (revoked/hết hạn → 401), rotate token (revoke cũ + mint mới).
- `/auth/logout` revoke thật bản ghi trong DB (trước đây chỉ trả `{loggedOut:true}` không làm gì).
- Verify: unit 113/113, e2e 36/36, curl thật replay cookie đã revoke → 401.
...
## Frontend — lưu token trong localStorage ✅ Hoàn tất (2026-07-22)

Đóng lại — không còn hoãn. App đã deploy production thật nên điều kiện trigger ban đầu đã tới.
- refreshToken chuyển sang httpOnly Secure(prod) SameSite=Lax cookie do backend set (path
  `/api/v1/auth`), frontend không còn thấy giá trị này.
- accessToken giữ **in-memory** (biến module-level `lib/auth-storage.ts`), không còn localStorage.
  Mất khi F5 → bootstrap lại qua `POST /auth/refresh` (cookie tự gửi kèm, `withCredentials:true`).
- Verify live qua browser thật: F5 giữ đăng nhập, logout xong truy cập lại → bounce `/login`.
