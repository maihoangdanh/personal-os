...
- [x] ✅ Session/refresh-token store để revoke được — bảng `RefreshToken`, rotate + revoke thật. Xem mục "Auth-hardening (2026-07-22)" bên dưới.
- [x] ✅ Chuyển token sang httpOnly cookie (thay localStorage) — refreshToken qua cookie, accessToken in-memory. Xem mục "Auth-hardening (2026-07-22)" bên dưới.
...
- [ ] 🔄 **Redesign: trục giờ + Agenda view** — thiết kế đã chốt qua brainstorm 2026-07-22, CHƯA
      code (dừng lại theo yêu cầu người dùng để làm tiếp sau). Chỉ cần sửa frontend (backend đã
      có sẵn `dateFrom/dateTo` filter cho cả Task lẫn CalendarEvent, không cần endpoint mới). Tóm
      tắt: 2 tab "Lưới" (giữ khung 7 cột hiện tại, thêm trục giờ 6:00–24:00, dải "không giờ" cho
      task không deadline) và "Agenda" (list theo ngày, tick hoàn thành task ngay tại chỗ, tái
      dùng optimistic update đã xây cho Dashboard "Hôm nay"). Chi tiết đầy đủ + các quyết định đã
      chốt: [docs/superpowers/specs/2026-07-22-calendar-timeline-agenda-design.md](docs/superpowers/specs/2026-07-22-calendar-timeline-agenda-design.md).
      Bước tiếp theo khi quay lại: xác nhận lại thiết kế → viết implementation plan → code.
...
- [x] ✅ `.env` production trên VPS đã xác nhận tách secret riêng (2026-07-22) — so hash SHA-256 của
      `JWT_ACCESS_SECRET`/`JWT_REFRESH_SECRET` giữa `.env.production` (VPS) và `.env` (local): khác
      hoàn toàn, không dùng chung với dev.
- [x] ✅ **CI/CD — hoàn tất (2026-07-22)**: `.github/workflows/ci.yml` — job `ci` (typecheck cả
      2 app, lint web, unit test API, `next build`) chạy trên push/PR vào `main`; job `deploy` (chỉ
      push trực tiếp `main`, sau khi `ci` xanh) SSH vào VPS, `git pull`, chỉ `docker compose
      build`/`up -d` đúng service bị đổi (so `git diff` đường dẫn `apps/web|Dockerfile.web` →
      `web`, `apps/api|Dockerfile.api|packages/database` → `api`; đổi file khác → bỏ qua, không
      rebuild), verify bằng curl `https://task.hoangdanh.cloud/dashboard` phải 200 mới coi là xong.
      e2e KHÔNG chạy trong CI (cần Supabase thật, rủi ro đụng data prod) — vẫn chạy tay khi cần.
      **Cần làm thủ công 1 lần**: thêm 3 GitHub Secrets vào repo settings — `SSH_HOST`
      (`149.118.63.154`), `SSH_USER` (`ubuntu`), `SSH_PRIVATE_KEY` (nội dung file
      `~/.ssh/oracle-personalos.key`) — chưa thêm thì job `deploy` sẽ fail tới khi có.
...
## Auth-hardening ✅ Hoàn tất (2026-07-22)

Làm ngay sau khi deploy production, đóng 2 mục hoãn có điều kiện "trước khi deploy production thật"
(điều kiện đã tới). Chạy qua `personal-os-team` skill: database-architect → backend-engineer →
frontend-engineer, devops-engineer làm CI/CD song song.

- [x] ✅ **DB**: model `RefreshToken` (`packages/database/prisma/schema.prisma`, migration
      `016_refresh_token`) — `userId` (FK Cascade), `tokenHash` (SHA-256 hex, unique — KHÔNG lưu
      plaintext), `expiresAt`, `revokedAt` (null = còn hiệu lực), index theo `userId`/`expiresAt`.
      Không unique 1-1 với User (nhiều thiết bị/tab cùng lúc).
- [x] ✅ **Backend** (`apps/api/src/auth/*`): login/register hash + lưu refresh token vào DB, set
      qua cookie `refreshToken` (httpOnly, `Secure` khi `NODE_ENV=production`, `SameSite=Lax`, path
      `/api/v1/auth`) thay vì trả trong JSON body. `/auth/refresh` đọc cookie (không nhận body nữa,
      `RefreshDto` đã xoá), verify JWT + tra DB (revoked/hết hạn → 401), **rotate** (revoke bản ghi
      cũ, mint + lưu bản ghi mới) — chống replay token cũ. `/auth/logout` revoke thật bản ghi DB +
      clear cookie (trước đây chỉ trả `{loggedOut:true}` không làm gì). `TokensDto.refreshToken`
      đổi optional, JSON body giờ chỉ còn `accessToken`. Unit **113/113 pass**, e2e **36/36 pass**,
      verify LIVE bằng curl: replay refresh cookie đã logout → **401** (chứng minh revoke DB thật).
- [x] ✅ **Frontend** (`apps/web/src/lib/auth-storage.ts`, `api-client.ts`, `AuthGate.tsx`,
      `app/page.tsx`, `features/auth/*`): bỏ hẳn `localStorage` cho token. accessToken giữ
      **in-memory** (biến module-level, đọc được ngoài React cho axios interceptor). refreshToken
      không còn ở frontend — cookie tự gửi kèm (`withCredentials:true`). F5/mở tab mới → accessToken
      mất (chỉ in-memory) → `AuthGate`/`app/page.tsx` gọi `POST /auth/refresh` lúc mount để bootstrap
      lại phiên từ cookie, có loading state chống flash-redirect sai. `tsc --noEmit` sạch. Verify LIVE
      qua browser thật: F5 giữ đăng nhập, logout → truy cập lại bounce `/login`, cookie cũ sau logout
      dùng lại → 401 thật (không chỉ xoá state phía client).
      Không cần Next.js rewrite proxy — cookie `SameSite=Lax` hoạt động tốt giữa `localhost:3000`
      (web) và `localhost:3001` (api) vì cùng site `localhost` (khác port, không khác site).
...
| 2026-07-22 | Bảng Task (`/tasks`) các nhóm project lệch cột (Status/Ưu tiên/Deadline không thẳng hàng) | Mỗi nhóm project render 1 `<table>` riêng, trình duyệt tự co giãn cột theo nội dung từng bảng | `table-fixed` + `colgroup` cố định độ rộng cột (commit `528c399`) |
| 2026-07-22 | Nút "Hoàn thành" ở Dashboard "Hôm nay" bấm không phản hồi | `useCompleteTask` không invalidate queryKey riêng `["dashboard","todayTasks"]` (không nằm trong `taskKeys.all`) nên list không refetch | Invalidate thêm key riêng (commit `2b7a7f4`) |
| 2026-07-22 | Dashboard "Hôm nay" hiện sai tên Goal (vd. hiện "Cá nhân" thay vì "Trioderma") | `goalKeys.goals()` chỉ encode `visionId` vào queryKey, thiếu `status` — hai nơi gọi `useGoals()` (tất cả) và `useGoals({status:"ACTIVE"})` (StatStrip/GoalProgressWidget) ra CÙNG queryKey, React Query gộp chung 1 query, lẫn kết quả 2 filter khác nhau | Thêm `status` vào key để tách riêng từng query (commit `1674075`) |

## Việc cần làm ngay tiếp theo (cập nhật 2026-07-22)

**Không còn gap nào trong roadmap 4 phase, và 3/4 việc ưu tiên đợt deploy production đã xong**
(httpOnly cookie, session/refresh revoke, CI/CD — xem mục "Auth-hardening" ở trên). Còn lại:

1. **Thêm 3 GitHub Secrets thủ công** để job `deploy` của CI/CD chạy được (`SSH_HOST`, `SSH_USER`,
   `SSH_PRIVATE_KEY`) — xem chi tiết ở mục "Deploy Oracle Cloud" phía trên. Việc duy nhất còn treo,
   không cần code thêm, chỉ cần vào GitHub repo settings.
2. **Calendar redesign (trục giờ + Agenda view)** — thiết kế đã chốt, chưa code, xem mục
   "Calendar" ở trên + spec đầy đủ tại `docs/superpowers/specs/2026-07-22-calendar-timeline-agenda-design.md`.
3. Backup Service cho VM/Docker — **quyết định không làm** (người dùng xác nhận 2026-07-22), rủi ro
   thấp vì dữ liệu chính ở Supabase đã có backup riêng.
4. Dùng thật hàng ngày để tinh chỉnh ngưỡng Eisenhower + phát hiện thêm bug ẩn (đã có 4 ca thật —
   xem bảng trên, 3/4 ca phát hiện ngay trong session deploy 2026-07-22).
